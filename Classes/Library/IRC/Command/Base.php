<?php
    ...
    abstract class Base {
       ...
       protected function say($msg) {
		...
		$this->say("1 Doge = " . $output . " bitcoins.");
         ...
         * @return string
         */
        protected function fetchjoke($uri) {

            $this->bot->log("Fetching from URI: " . $uri);

		$response = file_get_contents($uri);
		$object = json_decode($response);
		$output = $object->value->joke;
         ...
         * @return string
         */
        protected function fetchdiff($uri) {

            $this->bot->log("Fetching from URI: " . $uri);

		$response = file_get_contents($uri);
		$object = json_decode($response);
		$output = $object->data->difficulty;

            $this->bot->log("Data fetched: " . $output);
		$this->say("The current difficulty is: " . $output);
         ...
         * @return string
         */
        protected function fetchbtc($uri) {
		...
		$output = $object->value;
            ...
            $this->bot->log("Data fetched: " . $output);
		$this->say("1 Bitcoin is currently: " . "$" . $output);
         ...
         * @return string
         */
        protected function fetchtotal($uri) {
		...
		$object = json_decode($response);
		$output = $object->last;
		$total = $this->arguments[0];
		$output_output = $total * $output;
            $this->bot->log("Data fetched: " . $output_output);
		$this->say("Amount in bitcoins: " . $output_output);
